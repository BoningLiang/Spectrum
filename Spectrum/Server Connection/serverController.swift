            ...
            DispatchQueue.main.async {
                if let data = data{
                    do{
                                ...
                                for jsonCase in jsonCases{
                                    ...
                                    if let teachersNotes = jsonCase["teachersNotes"] as? [[String: Any]]{
                                        for teachersNote in teachersNotes{
                                            ...
                                            let resultTeachersNote = TeachersNote(noteID: noteID!,noteVideo: noteVideo!,noteCover: noteCover!,outCaseID: caseID!)
                                        ...
                                        for question in questions{
                                            ...
                                            if let options = question["options"] as? [[String:Any]]{
                                                for jsonOption in options{
                                                    ...
                                                    let resultJsonOption = Option(optionID: optionID!,optionContent: optionContent!,isSelect: isSelect!,isCorrect: isCorrect!,
                                                                                  outQuestionID: questionID!)
                                                    resultJsonOptions.append(resultJsonOption)
                                                }
                                            }
                                            let resultQuestion = Question(questionID: questionID!,question: questionContent!,options: resultJsonOptions,explanation: explanation,expanded: false,outCaseID: caseID!)
                        ...
                        print("ServerController: " + error.localizedDescription)
                    }
                    completion(AllCases)
                }
            }
